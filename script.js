...
async function setupWalletAfterConnected() {
  ...
  // Use the connected wallet provider for reads. This avoids mobile browser CORS/RPC issues.
...
async function loadContractData() {
  ...
  try {
    ...
    if (!code || code === "0x") {
      throw new Error("No contract found on BNB Smart Chain at this address. Check that the airdrop contract was deployed on BSC, not ETH/testnet.");
    }
...
async function claimAirdrop() {
  ...
  try {
    ...
    console.error(error);
    showMessage("Claim failed: " + getReadableError(error));
    claimBtn.disabled = false;
    claimBtn.innerText = "claim $SHIT";
  }
}
